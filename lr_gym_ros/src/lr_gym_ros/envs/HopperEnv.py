...
import lr_gym_ros.utils.PyBulletUtils as PyBulletUtils
...
class HopperEnv(ControlledEnv):
    ...
    def getUiRendering(self) -> Tuple[np.ndarray, float]:
        ...
        npArrImg = lr_gym_ros.utils.utils.image_to_numpy(imgMsg)
    ...
    def buildSimulation(self, backend : str = "gazebo"):
        if backend == "gazebo":
            ...
            if self._useMjcfFile:
                PyBulletUtils.buildSimpleEnv(rospkg.RosPack().get_path("lr_gym_ros")+"/models/hopper_mjcf_pybullet.xml",fileFormat = "mjcf")
            else:
                PyBulletUtils.buildSimpleEnv(rospkg.RosPack().get_path("lr_gym_ros")+"/models/hopper_v0_pybullet.urdf")
        else:
