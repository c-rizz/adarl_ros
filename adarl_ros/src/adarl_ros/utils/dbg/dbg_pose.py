...
class DbgPose:
    ...
    def publish(self, streamName : str, pose : adarl.utils.utils.Pose, frame_id : str = "world"):
        if not self._initialized:
            self.init()
        if streamName not in self._publishers:
            self._addDbgStream(streamName)
        if isinstance(pose, adarl.utils.utils.Pose):
            pose = pose.getPoseStamped(frame_id)
        elif isinstance(pose, geometry_msgs.msg.PoseStamped):
            pose = pose
        else:
            raise AttributeError(f"You can only use PoseStamped or adarl_ros Poses. pose is a {type(pose)}")
        for i in range(5):
