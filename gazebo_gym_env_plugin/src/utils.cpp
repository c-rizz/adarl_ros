...
std::string getCameraRosEncoding(std::shared_ptr<sensors::CameraSensor> sensor)
{
  ...
  {
    ROS_INFO_STREAM("adarl_ros_env: bayer simulation maybe computationally expensive.");
  ...
  {
    ROS_ERROR_STREAM("adarl_ros_env: Unsupported Gazebo ImageFormat "<<sensorFormat<<" on sensor "<<sensor->Name());
    ret = sensor_msgs::image_encodings::BGR8;
  }
