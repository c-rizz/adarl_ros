## Branches
These instructions assume you want to use tensorflow/stable_baselines2, if you want to use pytorch and stable_baselines3 
...
* adarl_ros is the main package, and provides the means to easily implement RL ROS environments,
...
* adarl_ros_utils provides various utilities for helping development. Important ones are:
...
## The adarl_ros package

In the adarl_ros/src/adarl_ros folder you can find python3 code for implementing ROS-based
RL environments.
...
There are various environments already implemented, you can find them in the `adarl_ros/src/lr_gyn/envs` folder.
 ...
 * This current repository (if you didn't do it already):
   ```
   git clone --branch a2110 https://gitlab.com/crzz/adarl_ros.git
   ```
 * For using the Panda arm you will need the `lr_panda` repository:
   ```
   git clone --branch a2110 https://gitlab.com/crzz/lr_panda.git
   ```
 * For using the Panda arm with moveit you will need `lr_panda_moveit_config`:
   ```
   git clone --branch a2110 https://gitlab.com/crzz/lr_panda_moveit_config.git
   ```
 * For using the realsense camera you will need `lr_realsense`:
   ```
   git clone --branch a2110 https://gitlab.com/crzz/lr_realsense.git
   ```

You will also need to install some python3 modules, preferably in a python virtual
environment. You can use the build_virtualenv.sh helper script in the adarl_ros folder.
...
```
rosrun adarl_ros test_cartpole_env.py
```
...
```
rosrun adarl_ros test_cartpole_env.py --render
```
...
```
rosrun adarl_ros solve_cartpole_sb3.py
```

To see the simulation you can launch a gazebo client using the following:

```
roslaunch adarl_ros gazebo_client.launch id:=0
```
...
```
rosrun adarl_ros solve_cartpole_sb3_a2c_vec.py --envsNum=4
```
...
```
rosrun adarl_ros solve_cartpole_sb2_sac_vec.py --envsNum=4
```
...
```
rosrun adarl_ros solve_hopper_sb3.py
```
...
As always the Gazebo simulation can be visualized using 'roslaunch adarl_ros gazebo_client.launch'
...
* **In simulation**:
  ```
  rosrun adarl_ros test_pandaMoveitPick.py
  ```
* **In the real** (substituting your robot ip):
  ```
  rosrun adarl_ros test_pandaMoveitPick.py --real --robot_ip x.x.x.x
  ```
 ...
 * PandaMoveitVarReachingEnv:
   ```
    rosrun adarl_ros solve_pandaMoveitVarReaching.py --load src/adarl_ros/adarl_ros/trained_models/pandaMoveitPoseReachingEnv320210309-184658s200000_62400_steps.zip
   ```
 * PandaEffortKeepPose:
   ```
    rosrun adarl_ros solve_panda_effort_keep_pose_vec.py --load solve_panda_effort_keep_tensorboard/20201114-212527/checkpoints/sac_pandaEffortKeep_20201114-212527s15000_3000000_steps.zip
   ```
...
```
rosrun adarl_ros plotGymEnvLogs.py --csvfile solve_cartpole_env/20210310-120235/GymEnvWrapper_log.csv
```

Check the `rosrun adarl_ros plotGymEnvLogs.py --help` for more info
